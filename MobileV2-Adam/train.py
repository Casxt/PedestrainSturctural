...
from tools import DataGenerator, loadMate, MultiLableLoss, MultiLableAccuracy

img_target_size = (224, 224, 3)
batch_size = 32
classes, img_path, img_label = loadMate()

checkPointer = ModelCheckpoint(filepath="./MobileV2-Adam/checkpoint/{epoch:02d}.hdf5",
                               save_best_only=False, verbose=1, period=1)

tensorBoard = TensorBoard(log_dir='./MobileV2-Adam/logs',  # log 目录
...
optimizers = keras.optimizers.Adam(lr=0.01, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.00001, amsgrad=False)
