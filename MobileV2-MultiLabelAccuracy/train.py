...
from tools import DataGenerator, loadMate, MultiLableLoss, MultiLableAccuracy, MultiLableAccuracyLoss, MultiLabelTensorBoard

img_target_size = (224, 224, 3)
batch_size = 32
classes, img_path, img_label = loadMate()

checkPointer = ModelCheckpoint(filepath="./MobileV2-MultiLabelAccuracy/checkpoint/{epoch:02d}.hdf5",
                               save_best_only=True, verbose=1, period=1)

tensorBoard = MultiLabelTensorBoard(log_dir='./MobileV2-MultiLabelAccuracy/logs',  # log 目录
                                    histogram_freq=0,
...
model.compile(optimizer=optimizers, loss=MultiLableAccuracyLoss,
              metrics=["categorical_accuracy", "mean_squared_error", "mean_absolute_error",
                       "mean_absolute_percentage_error", "categorical_crossentropy", MultiLableLoss, MultiLableAccuracy])
model.fit_generator(generator=train, epochs=250, verbose=1, validation_data=validation,
                    workers=4, initial_epoch=0, callbacks=[checkPointer, tensorBoard])
