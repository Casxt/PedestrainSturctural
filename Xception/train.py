...
# 指定显卡
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
img_target_size = (96, 288, 3)
batch_size = 32
classes, img_path, img_label = loadMate()

checkPointer = ModelCheckpoint(filepath="./Xception/checkpoint/{epoch:02d}.hdf5",
                               save_best_only=False, verbose=1, period=1)

tensorBoard = MultiLabelTensorBoard(log_dir='./Xception/logs',  # log 目录
...
base_model = Xception(weights=None, include_top=False,
                      input_shape=img_target_size)  # 'imagenet'
